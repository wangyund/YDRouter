 ...
 * 把一个类和一个地址绑定
 * @pram route 绑定的地址
 * @pram controllerClassName 绑定的类名
 */
- (void)map:(NSString *)route toControllerClassName:(NSString *)controllerClassName {
    NSMutableDictionary *subRoutes = [self subRoutesToRoute:route];
    subRoutes[@"_"] = NSClassFromString(controllerClassName);
}
...
- (NSArray *)pathComponentsFromRoute:(NSString *)route {
    ...
    for (NSString *pathComponent in url.path.pathComponents) {
        ...
        [pathComponents addObject:[pathComponent stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]]];
    }
